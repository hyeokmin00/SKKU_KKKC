...
def number_search(request):
    ...
    if request.method == "POST":
        ...
        # 데이터베이스에서 조회
        phone_numbers = Phone_numbers.objects.filter(
            phone_number=search_number)
        account_numbers = Account_numbers.objects.filter(
...
class SimilarityView(View):
    ...
    def post(self, request):
        ...
        for i in all_text_mails[:50]:
            # print(i.phone_number)
            db_value = np.array(ast.literal_eval(
                i.message_embedding[7:-1])).reshape(-1, 1)
            # print(np.array(ast.literal_eval(i.message_embedding)[7:-1]))
            np_value = np.array(eval(transcript[7:-1]))

            similarity_score = np.dot(np_value, db_value) / (
                np.linalg.norm(np_value) * np.linalg.norm(db_value))
            ...
            if similarity_score > similarity_threshold:
                ...
                if input_text not in [item["text"] for item in input_data
                                      ] and input_num not in [
                                          item["phone_number"]
        ...
        if similarity_scores:
            result_data = [{
                "text": item["text"],
                "sim": item["similarity"]
            } for item in top_similarity_scores]
            return render(
                request,
                "result_model_high.html",
                {
                    "inputs": input_data,
                    "result_cnt": result_cnt,
                    "cat": cat,
                    "similarities": top_similarity_scores,
                },
            )
